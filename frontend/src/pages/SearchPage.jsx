...
export default function SearchPage() {
  ...
  const CourseCard=({curso,inscrito})=>(
    <div className={`search-card ${inscrito?'search-card-inscrito':''}`}>
      <div className={`search-card-banner ${curso.imagen?'':(inscrito?'search-card-banner-inscrito':'search-card-banner-default')}`} style={curso.imagen?{backgroundImage:`url(${curso.imagen})`,backgroundSize:'cover',backgroundPosition:'center'}:undefined}>{!curso.imagen&&<div className="search-card-banner-icon">📚</div>}</div>
      <div className="search-card-body">
        ...
        <div className="search-card-footer">
          ...
          {inscrito?<button onClick={()=>navigate(`/course/${curso._id}`)} className="btn-go-course">✓ Ir al curso</button>
  ...
  return(
    <div className="page-container">
      <header className="topbar">
        ...
        <div className="search-input-wrapper"><span className="search-icon">🔍</span><input type="text" placeholder="Buscar cursos..." value={busqueda} onChange={e=>setBusqueda(e.target.value)} className="search-input"/></div>
      </header>
      {msg.texto&&<div className={`alert ${msg.tipo==='ok'?'alert-success':'alert-error'} page-banner-alert`}><span>{msg.texto}</span><button onClick={()=>setMsg({texto:"",tipo:""})} className="alert-close">✕</button></div>}
      <div className="page-content">
        {loading?<div className="loading-state"><p className="loading-text">Cargando cursos disponibles...</p></div>:(
          <>
            ...
            {disponibles.length===0&&inscritos.length===0&&<div className="empty-state"><div className="empty-state-icon">🔍</div><p className="empty-state-desc">{busqueda?`Sin resultados para "${busqueda}"`:'No hay cursos disponibles.'}</p></div>}
          </>
        )}
      </div>
    </div>

    ...
    <div className="app-layout">
      {modal&&<div className="modal-backdrop">
        <div className="modal-card">
          <div className="modal-header"><h3 className="modal-title">Nuevo usuario</h3><button onClick={()=>setModal(false)} className="modal-close-btn">✕</button></div>
      ...
      <aside className="sidebar">
        <div className="sidebar-header"><div className="sidebar-brand"><div className="sidebar-logo">🎓</div><div><p className="sidebar-title">AulaVirtual Pro</p><p className="sidebar-role">Administrador</p></div></div></div>
        <nav className="sidebar-nav">{[['👥','Usuarios','usuarios'],['📚','Cursos','cursos'],['📊','Reportes','reportes']].map(([ico,lbl,t])=><button key={t} onClick={()=>setTab(t)} className={`nav-item ${tab===t?'active':''}`}><span className="nav-icon">{ico}</span>{lbl}</button>)}</nav>
      ...
      <div className="main-content">
        ...
        <div className="page-body">
          {loading?<p className="loading-text">Cargando datos...</p>:(
            <>
              <div className="dashboard-stats">{[['👥',usuarios.filter(u=>u.rol==='alumno').length,'Alumnos','stat-icon-blue'],['👨‍🏫',usuarios.filter(u=>u.rol==='instructor').length,'Instructores','stat-icon-green'],['📚',cursos.filter(c=>c.activo).length,'Cursos activos','stat-icon-purple'],['👤',usuarios.length,'Usuarios total','stat-icon-amber']].map(([ico,val,lbl,icn])=><div key={lbl} className="stat-card"><div className={`stat-icon-box ${icn}`}>{ico}</div><div><p className="stat-value">{val}</p><p className="stat-label">{lbl}</p></div></div>)}</div>
              {tab==='usuarios'&&<div className="table-card"><table className="data-table"><thead><tr>{['Nombre','Email','Rol','Estado','Acciones'].map(h=><TH key={h} c={h}/>)}</tr></thead><tbody>{usuarios.map(u=>{const rc=rcCls(u.rol);return(<tr key={u._id}><TD c={<div className="cell-user"><div className="table-avatar">{u.nombre?.[0]?.toUpperCase()||'?'}</div><span className="table-strong">{u.nombre}</span></div>}/><TD c={u.email}/><TD c={<span className={`badge ${rc}`}>{u.rol}</span>}/><TD c={<span className={`badge ${u.activo?'badge-success':'badge-inactive'}`}>{u.activo?'Activo':'Inactivo'}</span>}/><TD c={<div className="cell-actions"><button onClick={()=>handleToggle(u._id)} className="icon-btn">{u.activo?'🔒':'🔓'}</button><button onClick={()=>handleDelU(u._id)} className="icon-btn icon-btn-danger">🗑️</button></div>}/></tr>);})}</tbody></table></div>}

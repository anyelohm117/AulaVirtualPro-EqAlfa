...
export default function ProgressPage() {
  ...
  const stats=[{ico:'📚',val:progresos.length,lbl:'Cursos inscritos',sub:`${cursosComp} completados`},{ico:'🏆',val:cursosComp,lbl:'Completados',sub:'al 100%'},{ico:'📝',val:resultados.length,lbl:'Quizzes realizados',sub:`${quizAprobados} aprobados`},{ico:'⭐',val:promedio,lbl:'Promedio',sub:'sobre 10'}];
    ...
    <div className="app-layout">
      <aside className="sidebar">
        <div className="sidebar-header">
          <div className="sidebar-brand"><div className="sidebar-logo">🎓</div><p className="sidebar-title">AulaVirtual Pro</p></div>
        </div>
        <nav className="sidebar-nav">{NAV.map(item=><button key={item.path} onClick={()=>navigate(item.path)} className={`nav-item ${item.path==='/progress'?'active':''}`}><span className="nav-icon">{item.icon}</span>{item.label}</button>)}</nav>
      ...
      <div className="main-content">
        ...
        <div className="page-body">
          {loading?<p className="loading-text">Cargando...</p>:(
            <>
              <div className="dashboard-stats">{stats.map((st,i)=><div key={st.lbl} className="stat-card"><div className={`stat-icon-box ${iconCls[i]}`}>{st.ico}</div><div><p className="stat-value">{st.val}</p><p className="stat-label">{st.lbl}</p><p className={`stat-subtext ${subCls[i]}`}>{st.sub}</p></div></div>)}</div>
